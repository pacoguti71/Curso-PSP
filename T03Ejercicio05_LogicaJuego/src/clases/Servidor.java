...
public class Servidor extends Thread {
    ...
    public void run() {
        ...
        try (BufferedReader entrada = new BufferedReader(new InputStreamReader(socketCliente.getInputStream()));
            ...
            if (metodo.equals("POST")) { // coordenadas=a5
                String partes[]=cuerpoMensaje.split("=");
                if (partes[0].equals("coordenadas") && partes.length==2) {
                    String coordenadas=partes[1].toUpperCase().trim();
                    partida.disparar(coordenadas);
                }
            }
